...
function createMockExtractRun(
  overrides: Partial<Extend.ExtractRun> = {}
): Extend.ExtractRun {
  return {
    object: "extract_run",
  ...
  describe("createAndPoll", () => {
    describe("with standard (untyped) request", () => {
      it("should create and poll until processed", async () => {
        const createResponse: Extend.ExtractRun = createMockExtractRun({
          status: "PROCESSING",
        });

        const retrieveResponse1: Extend.ExtractRun = createMockExtractRun({
          status: "PROCESSING",
        });

        const retrieveResponse2: Extend.ExtractRun = createMockExtractRun({
          status: "PROCESSED",
        });
