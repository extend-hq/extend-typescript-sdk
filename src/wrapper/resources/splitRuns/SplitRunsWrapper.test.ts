...
function createMockSplitRun(
  overrides: Partial<Extend.SplitRun> = {}
): Extend.SplitRun {
  return {
    object: "split_run",
...
describe("SplitRunsWrapper", () => {
  ...
  describe("createAndPoll", () => {
    it("should create and poll until processed", async () => {
      const createResponse: Extend.SplitRun = createMockSplitRun({
        status: "PROCESSING",
      });

      const retrieveResponse1: Extend.SplitRun = createMockSplitRun({
        status: "PROCESSING",
      });

      const retrieveResponse2: Extend.SplitRun = createMockSplitRun({
        status: "PROCESSED",
      });
    ...
    it("should return immediately if already processed on first retrieve", async () => {
      ...
      const retrieveResponse: Extend.SplitRun = createMockSplitRun({
        status: "PROCESSED",
      });
