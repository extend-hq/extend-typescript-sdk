...
function createMockClassifyRun(
  overrides: Partial<Extend.ClassifyRun> = {}
): Extend.ClassifyRun {
  return {
    object: "classify_run",
    ...
    dashboardUrl:
      "https://dashboard.extend.ai/classify-runs/classify_run_test123",
    usage: { credits: 1 },
    failureReason: null,
    failureMessage: null,
    parseRunId: null,
...
describe("ClassifyRunsWrapper", () => {
  ...
  describe("createAndPoll", () => {
    it("should create and poll until processed", async () => {
      const createResponse: Extend.ClassifyRun = createMockClassifyRun({
        status: "PROCESSING",
      });

      const retrieveResponse1: Extend.ClassifyRun = createMockClassifyRun({
        status: "PROCESSING",
      });

      const retrieveResponse2: Extend.ClassifyRun = createMockClassifyRun({
        status: "PROCESSED",
      });
    ...
    it("should return immediately if already processed on first retrieve", async () => {
      ...
      const retrieveResponse: Extend.ClassifyRun = createMockClassifyRun({
        status: "PROCESSED",
      });
