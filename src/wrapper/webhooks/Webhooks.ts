 ...
 */
function isSignedDataUrlPayload(
  payload: unknown,
): payload is SignedDataUrlPayload {
  if (typeof payload !== "object" || payload === null) {
    return false;
  }
  const p = payload as Record<string, unknown>;
  return (
    p.object === "signed_data_url" &&
...
export class Webhooks {
  ...
  public verifyAndParse(
    ...
    try {
      ...
      throw new WebhookParseError("Failed to parse webhook body as JSON");
    }
   ...
   * ```
   */
  public async fetchSignedPayload(
    event: WebhookEventWithSignedUrl,
  ): Promise<Extend.WebhookEvent> {
    ...
    try {
      ...
      if (typeof fullPayload !== "object" || fullPayload === null) {
        throw new WebhookPayloadFetchError(
          "Signed payload response was not a valid JSON object",
        );
      }
      ...
      const message =
        error instanceof DOMException && error.name === "AbortError"
          ...
          : error instanceof Error
            ...
            : "Unknown error";
      throw new WebhookPayloadFetchError(
        `Failed to fetch signed payload: ${message}`,
      );
    } finally {
  ...
  private verifySignature(
    ...
    if (!timestamp) {
      throw new WebhookSignatureVerificationError(
        "Missing x-extend-request-timestamp header",
      );
    }

    if (!signature) {
      throw new WebhookSignatureVerificationError(
        "Missing x-extend-request-signature header",
      );
    }
    ...
    if (maxAgeSeconds > 0) {
      ...
      if (age > maxAgeSeconds) {
        throw new WebhookSignatureVerificationError(
          `Request timestamp too old (${age}s > ${maxAgeSeconds}s)`,
        );
      }
      ...
      if (age < -60) {
        ...
        throw new WebhookSignatureVerificationError(
          "Request timestamp in the future",
        );
      }
    }
    ...
    // Normalize to lowercase hex for comparison (digest("hex") is already lowercase,
    // but the incoming signature header may have been uppercased by middleware)
    const normalizedSignature = signature.toLowerCase();

    // Use timing-safe comparison to prevent timing attacks.
    // Compare as hex-decoded buffers for constant-length comparison.
    const signatureBuffer = Buffer.from(normalizedSignature, "hex");
    const expectedBuffer = Buffer.from(expectedSignature, "hex");

    // A valid SHA-256 hex signature decodes to exactly 32 bytes
    if (signatureBuffer.length !== 32 || expectedBuffer.length !== 32) {
  ...
  private getHeader(headers: WebhookHeaders, name: string): string | undefined {
    // Direct lookup (fast path for frameworks that lowercase headers, e.g. Express)
    const direct = headers[name];
    if (direct !== undefined) {
      return Array.isArray(direct) ? direct[0] : direct;
    }

    // Case-insensitive fallback for frameworks that preserve original casing
